...
from stratum3 import stats
from stratum3 import conns
from stratum3 import exceptions
from stratum3 import signature
...
class Protocol(LineOnlyReceiver):
    ...
    def get_ident(self):
        ...
        return "%s:%s" % (
        self.proxied_ip or self.transport.getPeer().host, "%x" % id(self))
    ...
    def connectionMade(self):
        ...
        self.expect_tcp_proxy_protocol_header = self.factory.__dict__.get(
        ...
        conns.ConnectionRegistry.add_connection(self)
    ...
    def connectionLost(self, reason):
        ...
        conns.ConnectionRegistry.remove_connection(self)
    ...
    def writeJsonResponse(self, data, message_id, use_signature=False, sign_method='',
                          sign_params=[]):
        if use_signature:
            serialized = signature.jsonrpc_dumps_sign(self.factory.signing_key,
                                                      self.factory.signing_id, False, \
                                                      message_id, sign_method,
                                                      sign_params, data, None)
        else:
    ...
    def writeJsonError(self, code, message, traceback, message_id, use_signature=False,
    ...
    def process_response(self, data, message_id, sign_method, sign_params,
                         request_counter):
        self.writeJsonResponse(data.result, message_id, data.sign, sign_method,
                               sign_params)
        request_counter.decrease()

    def process_failure(self, failure, message_id, sign_method, sign_params,
                        request_counter):
        if not isinstance(failure.value, exceptions.ServiceException):
        ...
        if message_id is not None:
            ...
            if settings.DEBUG:
                ...
                tb = None
            self.writeJsonError(code, failure.getErrorMessage(), tb, message_id, sign,
                                sign_method, sign_params)
        ...
        for line in lines:
            ...
            if len(line) > self.MAX_LENGTH:
                ...
                try:
                    ...
                    log.warning(
                        "Failed message: %s from %s" % (str(exc), self._get_ip()))
    ...
    def lineReceived(self, line, request_counter):
        ...
        if msg_method:
            ...
            try:
                result = self.event_handler._handle_event(msg_method, msg_params,
                                                          connection_ref=self)
                ...
                if msg_id is None:
                    ...
                    # It's a RPC call
                    result.addCallback(self.process_response, msg_id, msg_method,
            ...
            if msg_error is not None:
                meta['defer'].errback(
                    exceptions.RemoteServiceException(msg_error[0], msg_error[1],
                                                             msg_error[2]))
            else:
    ...
    def rpc(self, method, params, is_notification=False):
        ...
        d = defer.Deferred()
        self.lookup_table[request_id] = dict(defer=d, method=method, params=params)
        return d
